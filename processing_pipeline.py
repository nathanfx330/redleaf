...
def extract_text_for_copying(doc_path: Path, file_type: str, start_page=None, end_page=None) -> str:
    """
    Extracts text for copying. For PDFs, it reads the file. For TXT and HTML files, it reads
    ...
    if file_type in ['TXT', 'HTML']:
        ...
        try:
            ...
            # For HTML, we now ignore page numbers as it's all one block
            if file_type == 'HTML':
                start_page = None
                end_page = None
...
def _extract_text_from_pipermail(html_content: str) -> str:
    ...
    # Use double newline to join the main parts for vertical separation
    return "\n\n".join(parts)
...
def _extract_text_with_block_separation(html_content: str) -> str:
    ...
    # Define block-level tags that should be separated
    ...
    for tag in soup.find_all(block_tags):
        # We get text and strip whitespace from the ends.
        # .get_text(separator=' ', strip=True) handles cases like <p>Hello <b>world</b></p> -> "Hello world"
        text = tag.get_text(separator=' ', strip=True)
        if text:
            text_blocks.append(text)
            
    # Join all collected text blocks with a double newline for vertical spacing
    return "\n\n".join(text_blocks)
    ...
    try:
        ...
        if doc_info['file_type'] == 'PDF':
            ...
            try:
                ...
                extracted_text = ""
                if parsing_mode == 'pipermail':
                    extracted_text = _extract_text_from_pipermail(content)
                else: # Default to generic
                    extracted_text = _extract_text_with_block_separation(content)
                
                # --- MODIFIED: No longer paginate HTML. Store as one block. ---
                if extracted_text.strip():
                    page_content_map = {1: extracted_text.strip()}
                    page_count = 1
                else:
                    page_content_map = {1: ""} # Ensure there's always a page 1, even if empty
                    page_count = 0
        ...
        cursor.execute("BEGIN")
        cursor.execute("UPDATE documents SET page_count = ? WHERE id = ?", (page_count, doc_id))
        ...
        if extracted_data["entities"]:
            ...
            for text, label in entities_list:
                ...
                if res:
                    entity_id_map[(text, label)] = res['id']
            ...
            relationships_to_insert = [ (entity_id_map[subj], entity_id_map[obj], phrase, doc_id, page_num) for subj, obj, phrase, page_num in extracted_data["relationships"] if subj in entity_id_map and obj in entity_id_map ]
...
# ... The rest of the file (discover_and_register_documents, update_browse_cache) remains unchanged ...
def discover_and_register_documents():
    ...
    try:
        ...
        registered_count = 0
        supported_patterns = ["*.pdf", "*.txt", "*.html"]
        all_files = []
        ...
        for file_path in all_files:
            ...
            if rel_path_str not in db_files or db_files.get(rel_path_str) != current_hash_str:
                ...
                cursor.execute(
                    """
                    INSERT INTO documents (relative_path, file_hash, file_type, status, status_message, file_size_bytes, processed_at) 
                    ...
                    """, 
                    (rel_path_str, current_hash_str, file_type, current_size)
                )
                cursor.execute("COMMIT")
                registered_count += 1
