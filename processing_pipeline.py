    ...
    try:
        ...
        if doc_info['file_type'] == 'PDF':
            ...
            try:
                ...
                return "ERROR_OPENING_FILE"
        elif doc_info['file_type'] == 'SRT':
            try:
                content = full_path.read_text(encoding='utf-8', errors='ignore')
                
                # Parse into structured cues for the new table
                parsed_cues = _parse_srt_for_db(content)
                
                # Extract the clean text for FTS and relationship analysis
                extracted_text = _extract_text_from_srt(content)
        ...
        cursor.execute("BEGIN")
        # Clear old data for the document
        ...
        # Insert new structured SRT data if applicable
        if doc_info['file_type'] == 'SRT' and 'parsed_cues' in locals() and parsed_cues:
            cues_to_insert = [
                (doc_id, cue['sequence'], cue['timestamp'], cue['dialogue'])
                for cue in parsed_cues
            ]
            cursor.executemany(
                "INSERT INTO srt_cues (doc_id, sequence, timestamp, dialogue) VALUES (?, ?, ?, ?)",
                cues_to_insert
            )
