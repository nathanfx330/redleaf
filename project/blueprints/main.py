...
def entity_detail(label, text):
    ...
    sql_query = """
        SELECT 
            ...
            (SELECT page_content FROM content_index ci WHERE ci.doc_id = d.id AND ci.page_number = 
                (CASE WHEN d.file_type IN ('SRT', 'HTML') THEN 1 ELSE ea.page_number END)
            ) as page_content,
            (SELECT COUNT(*) FROM document_comments WHERE doc_id = d.id) as comment_count,
            (SELECT 1 FROM document_curation WHERE doc_id = d.id AND user_id = ?) as has_personal_note,
            (SELECT 1 FROM document_tags WHERE doc_id = d.id LIMIT 1) as has_tags,
    ...
    """
    db_results = db.execute(sql_query, (g.user['id'], entity['id'])).fetchall()
    
    results = []
    for row in db_results:
        row_dict = dict(row)
        content_for_snippet = row_dict.pop('page_content', '')

        if row_dict.get('file_type') == 'SRT':
            cue_num = row_dict['page_number']
            cue_content_row = db.execute(
                "SELECT dialogue FROM srt_cues WHERE doc_id = ? AND sequence = ?",
                (row_dict['doc_id'], cue_num)
            ).fetchone()
            if cue_content_row:
                content_for_snippet = cue_content_row['dialogue']
