...
def report_view(report_id):
    ...
    if doc_id_to_load:
        doc_to_load = db.execute(
            "SELECT id, file_type FROM documents WHERE id = ?", (doc_id_to_load,)
        ).fetchone()

    form = SecureForm()
    return render_template('synthesis.html', report=report, form=form, doc_to_load=doc_to_load)