...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (metadataTab) {
        ...
        saveBtn.addEventListener('click', async () => { const newCslData = buildCslFromForm(); const cslJsonString = JSON.stringify(newCslData, null, 2); saveBtn.disabled = true; saveStatus.textContent = "Saving..."; saveStatus.style.opacity = '1'; try { const response = await fetch(`/api/document/${docId}/metadata`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRFToken': CSRF_TOKEN }, body: JSON.stringify({ csl_json: cslJsonString }) }); const result = await response.json(); if (!response.ok) throw new Error(result.message); saveStatus.textContent = "Saved successfully."; setTimeout(() => { saveStatus.style.opacity = '0'; }, 2000); } catch (error) { saveStatus.textContent = `Error: ${error.message}`; } finally { saveBtn.disabled = false; } });
    ...
    if (fileType === 'SRT') {
        ...
        const renderLinkedState = (type, path, source) => {
            ...
            mediaSyncPanel.innerHTML = `<p class="text-muted" style="color: #4ADE80;">✅ ${mediaType} is linked.</p><div>${sourceIndicator} <code style="font-size: 0.9em; word-break: break-all;">${path}</code></div><button id="unlink-media-btn" class="button button-danger button-small mt-2">Unlink Media</button>${statusCheckHtml}`;
            if (xmlSyncPanel) {
        ...
        const renderUnlinkedState = () => {
            ...
            if (xmlSyncPanel) {
                ...
                if (scanBtn) scanBtn.disabled = false; // Always allow scanning
        ...
        const checkMediaStatus = async () => {
            try {
                ...
                if (result.linked) renderLinkedState(result.type, result.path, result.source); else renderUnlinkedState();
            } catch (error) {
        ...
        if (xmlSyncPanel) {
            ...
            const renderXmlMatchList = (result) => {
                const matches = result.matches, scannedCount = result.xml_files_scanned;
                if (matches.length === 0) { statusContainer.innerHTML = scannedCount > 0 ? `<p class="text-muted">Scanned ${scannedCount} XML file(s), but no potential episode entries were found.</p>` : '<p class="text-muted">No .xml files were found in your documents directory to scan.</p>'; return; }
                let listHtml = `<p>Found ${matches.length} potential match(es) from ${scannedCount} XML file(s).</p><ul class="simple-list">`;
                matches.forEach(match => {
                    const { preview, enclosure_url } = match; const title = preview.title || '<em>No Title</em>'; const author = preview.author ? (preview.author[0].literal || '') : '<em>No Author</em>'; const date = preview.issued ? preview.issued['date-parts'][0].join('-') : '<em>No Date</em>';
                    let linkButtonHtml = enclosure_url ? `<button class="button button-small link-from-web-btn" data-url="${enclosure_url}" title="Link directly from: ${enclosure_url}">Link from Web</button>` : '';
                    listHtml += `<li style="padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 4px; margin-bottom: 0.5rem;"><div style="display: flex; justify-content: space-between; align-items: center;"><div><strong>${title}</strong><p class="text-muted mb-0" style="font-size: 0.9em;">By ${author} on ${date} <br><code style="font-size: 0.8em;">Found in: ${match.xml_path}</code></p></div><div class="toolbar" style="gap: 0.5rem;">${linkButtonHtml}<button class="button button-small import-xml-btn" data-xml-path="${match.xml_path}" data-item-hash="${match.item_hash}">Import Metadata</button></div></div></li>`;
                });
                listHtml += '</ul>'; statusContainer.innerHTML = listHtml;
            };
